...
namespace ConsoleApp.VideoFiles.VideoForWindows
{
	public class AudioStream : AviStream{
		...
		private Avi.AVISTREAMINFO GetStreamInfo(IntPtr aviStream){
			...
			var result = Avi.AVIStreamInfo(aviStream, ref streamInfo, Marshal.SizeOf(streamInfo));
			if(result != 0) {
		...
		public Avi.PCMWAVEFORMAT GetFormat(){
			...
			var result = Avi.AVIStreamReadFormat(aviStream, 0, ref format, ref size);
			return format;
		}
		...
		public IntPtr GetStreamData(ref Avi.AVISTREAMINFO streamInfo, ref Avi.PCMWAVEFORMAT format, ref int streamLength){
			...
			var result = Avi.AVIStreamRead(aviStream, 0, streamLength, waveData, streamLength, 0, 0);
			if(result != 0){
