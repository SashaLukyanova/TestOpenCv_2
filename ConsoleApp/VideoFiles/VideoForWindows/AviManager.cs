...
namespace ConsoleApp.VideoFiles.VideoForWindows
{
	public class AviManager
	{
		...
		public VideoStream GetVideoStream(){
			...
			var result = Avi.AVIFileGetStream(
			                                  aviFile,
			                                  out aviStream,
        ...
        public void AddAudioStream(AudioStream newStream, int startAtFrameIndex) {
            ...
            IntPtr aviStream;
			var result = Avi.AVIFileCreateStream(aviFile, out aviStream, ref streamInfo);
			if(result != 0){
			...
			if(result != 0){
				throw new Exception("Exception in AVIStreamSetFormat: "+result);
			}
			
			result = Avi.AVIStreamWrite(aviStream, 0, streamLength, waveData, streamLength, Avi.AVIIF_KEYFRAME, 0, 0);
			if(result != 0){
				throw new Exception("Exception in AVIStreamWrite: "+result);
			}
			
			result = Avi.AVIStreamRelease(aviStream);
			if(result != 0){
				throw new Exception("Exception in AVIStreamRelease: "+result);
			}
        ...
        public AviManager CopyTo(String newFileName, float startAtSecond, float stopAtSecond) {
            ...
            try {
				...
				var startByteIndex = (int)( startAtSecond * (float)(waveStream.CountSamplesPerSecond * streamFormat.nChannels * waveStream.CountBitsPerSample) / 8);
				var stopByteIndex = (int)( stopAtSecond * (float)(waveStream.CountSamplesPerSecond * streamFormat.nChannels * waveStream.CountBitsPerSample) / 8);
