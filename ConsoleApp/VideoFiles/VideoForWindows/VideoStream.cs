...
namespace ConsoleApp.VideoFiles.VideoForWindows
{
	public class VideoStream : AviStream
	{
        ...
        public Int16 CountBitsPerPixel => countBitsPerPixel;
		...
		/// <param name="writeCompressed">true: create a compressed stream before adding frames</param>
		/// <param name="frameRate">Frames per second</param>
		/// <param name="firstFrameBitmap"></param>
		...
		private PixelFormat ConvertBitCountToPixelFormat(int bitCount){
			String formatName;
			if(bitCount > 16){
				formatName = $"Format{bitCount}bppRgb";
			}else if(bitCount == 16){
				formatName = "Format16bppRgb555";
			}else{ // < 16
				formatName = $"Format{bitCount}bppIndexed";
			}
		...
		private Avi.AVISTREAMINFO GetStreamInfo(IntPtr aviStream){
			...
			var result = Avi.AVIStreamInfo(StreamPointer, ref streamInfo, Marshal.SizeOf(streamInfo));
			if(result != 0) {
        ...
        private void CreateStreamWithoutFormat() {
            ...
            var result = Avi.AVIFileCreateStream(aviFile, out aviStream, ref strhdr);
		...
		private void CreateCompressedStream(){
			...
			var opts = new Avi.AVICOMPRESSOPTIONS();
            ...
            var result = Avi.AVIMakeCompressedStream(out compressedStream, aviStream, ref compressOptions, 0);
            if(result != 0) {
        ...
        private void CreateCompressedStream(Avi.AVICOMPRESSOPTIONS options) {
            var result = Avi.AVIMakeCompressedStream(out compressedStream, aviStream, ref options, 0);
            if (result != 0) {
                throw new Exception("Exception in AVIMakeCompressedStream: " + result);
            }
		...
		public void AddFrame(Bitmap bmp){
            ...
            var result = Avi.AVIStreamWrite(writeCompressed ? compressedStream : StreamPointer,
                                            countFrames, 1, 
                                            bmpDat.Scan0, 
		...
		private void SetFormat(IntPtr aviStream){
			...
			var result = Avi.AVIStreamSetFormat(aviStream, 0, ref bi, bi.biSize);
		...
		public Bitmap GetBitmap(int position){
            ...
            var dib = Avi.AVIStreamGetFrame(getFrameObject, firstFrame + position);
